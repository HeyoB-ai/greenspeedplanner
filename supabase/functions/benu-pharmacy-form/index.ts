// ════════════════════════════════════════════════════════════════════════
// Greenspeed Planner — BENU: de reactiepagina voor de apotheek
// ════════════════════════════════════════════════════════════════════════
// Supabase Edge Function (Deno). De apotheek klikt vanuit haar mailbox en
// reageert zonder in te loggen op de extra tijd die de koerier heeft gemeld.
// Zelfde opzet als extra-work: de benu-tabellen hebben geen enkele policy en
// geen rechten voor anon, dus dit is de enige weg erheen. Het token bepaalt
// welke ene melding bereikbaar is.
//
//   GET  ?t=<pharmacy_token>          → de melding, ook ná een antwoord
...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    const { data, error } = await admin.rpc('benu_pharmacy_by_token', { p_token: token });
    if (error) {
      console.error('[benu-apotheek] ophalen mislukt:', error.message);
  ...
  if (req.method === 'POST') {
    ...
    if (typeof body.approve !== 'boolean') {
      return json({ error: 'Kies akkoord of niet akkoord.' }, 400);
    }

    const note = typeof body.note === 'string' ? body.note.slice(0, 500) : null;

    const { error } = await admin.rpc('benu_pharmacy_respond', {
    ...
    if (error) {
      ...
      // 45xxx: het token klopt, maar er valt niets meer te beslissen. `closed`
      // vertelt de pagina dat de knoppen weg moeten.
      if (error.code?.startsWith('45')) {
        console.log('[benu-apotheek] afgesloten:', error.code, error.message);
        return json({ error: error.message, closed: true }, 409);
      }
      console.warn('[benu-apotheek] antwoord geweigerd:', error.message);
      return json({ error: error.message }, 400);
    }

    const { data } = await admin.rpc('benu_pharmacy_by_token', { p_token: token });
    const row = Array.isArray(data) ? data[0] : data;
    return json({ ok: true, entry: row ?? null }, 200);
  }
